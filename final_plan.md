...
Synthea also emits structured FHIR, the same data yields **free ground truth** for
...
~30 diabetic pts  →   reads corpus + queries from S3   →   FAISS + optional BM25 →  Claude Haiku 4.5  →  vs FHIR gold
...
| 02 | Gold labels from FHIR (answer truth + gold chunk IDs) | 3.5 |
...
| Risk | Fallback |
|---|---|
| Stage 02 FHIR traversal overruns (cap at 4h) | Hardcode gold labels for 5 patients; shrink query set accordingly. |
